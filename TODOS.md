...
- **Детерминированный нормализатор реализован (2026-07-27), конвейер `n1`.**
  `Sources/VoiceType/Services/LexiconNormalizer.swift` — закрытый список из 139 точных
  словоформ, спановый матчер по токенам с longest-match-first. Замер на `eval-ru`:
  **dev — `propn` 43/43, `handoff` 12/12, `loanword` 4/4, `doubling` 1/1, неверных
  исправлений 0, регрессий 0, collateral damage 0.00%**; holdout — `propn` 12/15,
  неверных 0, регрессий 0, collateral 0.00%.
  Ключевое решение: **никаких префиксных правил**. `хенд*` превратил бы `хендлеры`
  (handlers) в `handoff`, `опус*` — `опустим` в `Opus` (а `опуст*` встречается 4 раза
  в самом эталонном корпусе), подстрочная замена сделала бы из `допустим` — `дOpusтим`.
  Незнакомая форма пропускается: пропуск дёшев, неверная замена дорога.
  Не закрыто нормализатором и осталось у LLM: **филлеры (33 из 96 изменяемых спанов
  dev — самая крупная категория корпуса)** и пунктуация (3 спана); «вот», «значит»,
  «короче» различаются как частица и знаменательное слово только по контексту.
  Снапшот всех замен на продовом корпусе:
  [`scripts/bench-output/RESULTS-normalizer-snapshot-2026-07-27.md`](scripts/bench-output/RESULTS-normalizer-snapshot-2026-07-27.md).
- **Филлеры закрыты детерминированно (2026-07-27), конвейер `n2`.**
  `Sources/VoiceType/Services/FillerRemover.swift` удаляет «вот», «ну», «короче»
  при пяти обязательных условиях: строчная буква, не начало предложения, для
  «вот» — следующее слово не «так» и не «и», вне защищённых зон, не часть
  дефисного композита. Замер продовым кодом на `eval-ru`: **dev — filler 30/33
  (91%), неверных исправлений 0, регрессий 0, collateral damage 0.00%**;
  holdout — 2/2, collateral 0.38% (два спорных удаления, вынесены владельцу).
  Словарь после смены порядка стадий не пострадал: propn 43/43, handoff 12/12.
  На продовом корпусе — 1541 удаление (вот 983, ну 488, короче 70) в 43% записей.
  **Порядок стадий изменён на «филлеры → словарь»**: филлер между частями
  составной словарной формы («сонет ну агенты») иначе прячет её от словаря;
  на корпусе таких случаев 0, но защита бесплатна, а отказ был бы молчаливым.
  **Не закрыты и почему:** «значит» (слева «это»×6, «есть»×4 — глагол или
  «следовательно»), «типа» (предлог: «группу типа read-only»), «как бы»
  (модальность), «э-э» (0 вхождений). Ни «значит», ни «типа» не размечены ни в
  одном спане корпуса — исключение бесплатно по метрике.
  **Тумблер «Remove filler words» выключен по умолчанию** (решение 2026-07-27
  после того, как владелец отложил аудит): слой удаляет слова, а его точность на
  реальной речи держится на четырёх наблюдениях holdout. Включается в
  Settings → General → INSERTION.
  **Открыто:** выборочный аудит 237 удалений — `~/Desktop/filler-audit-2026-07-27.md`
  (файл локальный, в git не уходит; генерируется `scripts/filler-audit.py --sample`).
  Разметить `[y]`/`[x]`, затем `--score`. Гейт: ≥0.98 — переключить дефолт на
  включённый; 0.95–0.98 — ужесточить по слову с худшим профилем; <0.95 —
  оставить только бесспорное подмножество.
- **Обязательный UX-элемент закрыт (2026-07-27): тумблер «Normalize terminology»** в
  Settings → General → INSERTION, под «Trim whitespace» (`AppSettings.normalizeTranscript`,
  по умолчанию включён). Гейтит **только словарь**: фильтр галлюцинаций тумблера не
  получает намеренно — он снимает текст, который пользователь не произносил, и режима,
  в котором его хочется вставить, не существует. Штамп конвейера при выключенном
  тумблере получает суффикс `-nolex` (`n1-nolex:p…`), иначе история приписывала бы
  записи словарный проход, которого не было. Второй обязательный элемент из этого же
  пункта — показ сырого текста в History рядом с обработанным — **остаётся открытым**:
  `Entry.rawText` пишется с конвейера `n1` (29.9% записей), но UI его не показывает.
...
## T16 — Whisper галлюцинирует «Продолжение следует…» на коротком аудио ✅ DONE (2026-07-27)

- **Outcome:** `Sources/VoiceType/Services/HallucinationFilter.swift` — закрытый список
  шаблонов субтитров, снимаемый **непрерывными сериями с обоих краёв** массива сегментов
  whisper; сегмент в строгой середине не удаляется никогда. На всех 1000 записях
  переразметки: **19 из 19 вычищено, 0 пропусков, 0 ложных срабатываний**.
- **Две гипотезы из постановки опровергнуты замером, порогов в реализации нет:**
  - порог по длительности не работает — галлюцинации встречаются на аудио от 0.61 с
    до 60.21 с, при этом есть осмысленная запись «По поводу.» на 0.92 с;
  - порог по плотности символов не работает — у галлюцинаций 1.02–36.18 симв/с против
    0.91–23.15 у настоящей речи, диапазоны накрывают друг друга полностью. Прежняя
    оценка «33–36 против 20–22» была артефактом восьми коротких примеров.
- **Правило сначала сузили до суффикса** (по замечанию план-ревью), но замер показал,
  что суффикс пропускает запись `Продолжение следует...` + `Спасибо.`, где шаблон стоит
  первым. Вариант «серии с обоих краёв» даёт recall 100% и сохраняет защиту середины.
- **Фильтр работает на уровне сегментов, а не строк текста:** вывод приложения не
  содержит `\n` (0 из 100 пар и 0 из 100 записей истории), переводы строк рисует сам
  `whisper-cli` при печати. Построчный фильтр прошёл бы тесты на дампах и молча не
  сработал бы в проде.
- **Когда снят весь вывод**, текст становится пустым и срабатывает существующая ветка
  `.emptyResult` — нового UI не потребовалось. Факт снятия пишется в `errors.log`
  (запись в историю не делается: она дала бы пустую строку в списке History).

## T16 — исходная постановка (для истории)
