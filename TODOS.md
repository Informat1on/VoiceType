...
- **Исследовательская часть закрыта (2026-07-27), UI-часть остаётся.** Порог найден
  в исходниках форка: `max_prompt_ctx = min(n_max_text_ctx, n_text_ctx/2)`
  (`whisper.cpp/src/whisper.cpp:6926`), а `n_text_ctx = 448` для всех моделей
  whisper (`:596`) — то есть **224 токена**. Догадка «likely trim at ~224 tokens»
  подтвердилась.
  Два следствия, которых в задаче не было:
  1. **При переполнении отбрасывается НАЧАЛО, а не конец** — используются последние
     `max_prompt_ctx - 1` = 223 токена (`:6950`, с предупреждением в лог
     `initial prompt is too long`). Значит приоритетные термины в промпте должны
     стоять в конце, а не в начале, и clamp в UI должен обрезать слева.
  2. Кириллица в BPE whisper дороже латиницы примерно вдвое-втрое на слово, поэтому
     clamp по числу символов будет врать. Честный счётчик — по токенам, но
     тогда нужен доступ к `whisper_tokenize` из UI.
  Плюс рядом найден `carry_initial_prompt` (`whisper.h:528`) — переносит промпт в
  каждое окно декодирования; см. замер в отчёте A/B по промпту.
...
- **Проверка ещё трёх моделей (2026-07-27, по предложению владельца):** `Qwen3.5-0.8B-4bit` (635 МБ) — 45% дефектов, четверть катастрофические (отвечает на транскрипт вместо правки, цитирует промпт, на длинном тексте сфабриковала эссе в 6.3 раза длиннее входа); `Qwen3.5-2B-4bit` (1.6 ГБ) — катастроф нет за 60 прогонов, но 65% дословного эха; `gemma-4-e2b-it-4bit` (3.3 ГБ) — активнее, но переводит цифры в слова вопреки правилу и один раз выдала метаответ. Латентность на самом длинном реальном тексте (2250 символов): 9.5 / 3.2 / 4.3 с — ни одна не укладывается в бюджет. **Общее для всех четырёх проверенных моделей: ни одна ни разу не нормализовала спрягаемые глагольные кальки («коммитил, пушил»), хотя существительные («гитхабе» → GitHub) нормализует.** То есть ключевая способность, ради которой задача затевалась, не подтвердилась.
- **Две альтернативные квантизации Qwen3.5-2B (2026-07-27):** гипотеза «меньше размер = выше скорость» **не подтвердилась**. `mxfp4` (0.98 ГБ, на 40% легче базовой 1.6 ГБ) даёт ту же скорость в пределах шума (0.256 с против 0.26 с; 3.10 с против 3.2 с), но втрое больше дефектов и систематический баг «Смотри» → «Смотря». `OptiQ-4bit` (2.0 ГБ) в текущем mlx-swift **вообще не работает** — генерирует нечитаемый мусор на всех 20 записях (смешанная точность по тензорам применяется рантаймом некорректно). При одинаковой архитектуре скорость определяется не размером файла, а тем, как формат деквантизации ложится на Metal-кернелы.
- **Проверка промпта с явной конвенцией (2026-07-27):** после консультации проверен промпт, признающий кириллические глагольные формы нормой. На базовой модели доля дословного эха упала с 65% до 50% — снятие внутреннего противоречия инструкции действительно заставляет модель работать. Но промпт применил правило к именам собственным («сонета агента»), сломав падежи; нужна формулировка с явным исключением имён.
...
- **Конвенция зафиксирована (2026-07-27):** [`docs/decisions/2026-07-27-transcript-postprocessing-convention.md`](docs/decisions/2026-07-27-transcript-postprocessing-convention.md) — что считается правильным выходом для каждой категории, на корпусе из 1096 надиктованных записей (90 136 слов). Без этого документа замер качества неинтерпретируем — это главный вывод измерений выше. Приоритеты по измеренному объёму получились иными, чем предполагал WER-бенч: числа словами 409 вхождений, имена собственные кириллицей ~340, `handoff` 106 (в 24 написаниях), а спрягаемые кальки — всего ~8, потому что whisper пишет их правильно в 97% случаев. Прежняя оценка «числа 59.2%» — артефакт эталонов `Tests/Fixtures/bench/`, где числа записаны словами.
...
## T16 — Whisper галлюцинирует «Продолжение следует…» на коротком аудио (P2)

- **What:** отфильтровать вывод whisper, когда он состоит из шаблонной фразы,
  характерной для субтитров, и/или ввести порог по длительности и уровню сигнала
  перед отправкой аудио в распознавание.
- **Why:** найдено 2026-07-27 на переразметке архива аудио: из 406 файлов **8
  (2.0%)** дали вывод «Продолжение следует…» или «Продолжение следует.», в том
  числе на записях длительностью **0.61 с**. Это артефакт ютубовских субтитров в
  обучающих данных whisper, известный класс галлюцинации на тишине.
  Практическое следствие: пользователь нажал хоткей, ничего не сказал или сказал
  полслова — и в редактор вставилась фраза, которой он не произносил. Это хуже,
  чем не вставить ничего.
- **Почему сейчас не прикрыто:** защита есть только от **буквально пустой**
  строки — `guard !text.isEmpty` в `AppDelegate.swift:961`, дальше состояние
  капсулы `.emptyResult`. Галлюцинация непустая, поэтому проходит насквозь.
  Порога по тишине или RMS в `AudioCaptureService` нет вовсе.
- **Pros:** дешёвый фикс с большим эффектом на доверие к продукту; закрывает
  целый класс «приложение написало то, чего я не говорил».
- **Cons:** список шаблонных фраз — открытый класс, полностью его не закрыть;
  порог по длительности рискует отсечь настоящие короткие команды («да»,
  «отмена»), поэтому его надо выбирать по данным, а не наугад.
- **Context:** в корпусе есть и обратный пример — запись 0.92 с с осмысленным
  «По поводу.», то есть отсекать всё короткое нельзя. Разумный первый шаг —
  закрытый список шаблонов субтитров плюс проверка, что вывод не длиннее
  разумного для своей длительности (у галлюцинаций 33-36 символов в секунду
  против 20-22 у настоящей быстрой речи).
- **Depends on:** независимо. Данные для выбора порогов — `retranscription.jsonl`
  с полями `duration` и `chars` по 406 файлам.
