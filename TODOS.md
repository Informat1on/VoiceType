...
## T11 — LLM-постобработка транскрипта локальной моделью (отдельный трек)
...
- **Depends on:** независим от трека оптимизации Whisper (T12); желательно после него, т.к. освободившийся бюджет латентности (см. T12: 13.2 с → 1.6 с на 90 с аудио) — это то, из чего оплачивается LLM-проход.

## T12 — Whisper backend: починить Metal + апгрейд whisper.cpp + Neural Accelerators M5

- **What:** три независимых шага, см. `scripts/bench-output/RESULTS-m5-backend-2026-07-26.md`: (a) починить доставку `ggml-metal.metal` в .app — сейчас Metal-бэкенд не инициализируется вообще; (b) поднять whisper.cpp в форке `Informat1on/SwiftWhisper` с v1.7.5 на v1.9.x; (c) включить Metal 4 tensor API (нейроакселераторы M5).
- **Why:** измерено на M5 Pro, turbo-q5_0, 90 с аудио: сейчас 13.2 с, после (a) 2.6 с, после (b) 2.2 с, после (c) 1.6 с.
- **Depends on:** (a) — самостоятельный багфикс, делать первым; (b) — крупная работа по форку (список исходников в `Package.swift` переписывается под новую структуру ggml); (c) — только после (b).
