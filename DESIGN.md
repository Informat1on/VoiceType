...
**Tab 1 — General:** LANGUAGE (segmented RU/RU+EN/EN/AUTO, default RU+EN) · INSERTION (Insert toggle, Trim whitespace, Normalize terminology, Remove filler words, Paste method) · MICROPHONE (device select + inline permission hint row).
...
| macOS audio service unresponsive (coreaudiod) | Toast: "macOS audio isn't responding · The system audio service is stuck. Restart it in Terminal: sudo killall -9 coreaudiod — or restart your Mac." | `sudo killall -9 coreaudiod` or restart the Mac; VoiceType recovers on its own once the demon comes back |
| Previous mic session stuck | Toast: "Microphone session is stuck · macOS hasn't released the previous recording session yet." | Try again in a moment, or quit and reopen VoiceType |
...
| 2026-07-26 | **Accelerator mode surfaced as a 3-state segmented control (Auto / Neural Engine / GPU) in Models → Core ML — deliberate departure from the toggle DESIGN.md:187 reserved for this slot** | `AppSettings.coreMLMode` (auto/on/off) doesn't fold into a 2-state toggle: `off` pins GPU outright, it isn't just "CoreML disabled" vs "enabled". `CoreMLMode.displayName`/`longDisplayName` renamed from the engineering draft ("Auto (Recommended)" / "Always On" / "Always Off") to name the chip, not the settings state — "Auto" / "Neural Engine" / "GPU", mirroring the short/long split already used by `Language`. Changing the segment calls the existing `AppDelegate.reloadModelForAccelerationModeChange()` so the new mode takes effect via the model-load queue immediately, not only after an app restart. |
| 2026-07-26 | **CoreML encoder badge, footnote, and Download button label now reflect the live `AccelerationDecision`, not encoder-file presence** | The footnote used to claim "GPU acceleration is enabled via CoreML" whenever the encoder file merely existed on disk — false whenever the resolved policy skips CoreML despite the file being there (mode Off), and equally false whenever Metal itself failed to initialize. `SettingsView` now probes `AcceleratorCapabilityProvider.shared.current()` once per Models-tab visit via `.task {}` into `@State` (the provider caches the answer for the process lifetime, so repeat visits resolve instantly), and derives the CoreML badge, the footnote text, and the Download button's "(with Neural Engine)" suffix from `AccelerationPolicyResolver.resolve(...)` / `.shouldInstallCoreML(...)`. A resolved GPU-only outcome (mode Off, or a model that ships no encoder) no longer paints the footnote in warning amber — it's a normal, often-faster result, not a problem state. **Follow-up (code review):** the `.metalOnly` branch still labelled the footnote "GPU" even when the resolved policy was `.metalOnly` purely because no CoreML encoder was installed while the hardware probe had come back `AcceleratorCapability.unavailable` — that combination actually runs on plain CPU. The label now checks `acceleratorCapability == .unavailable` and says "CPU" in that case instead of lying "GPU". |
| 2026-07-26 | **small-q5_1 + Neural Engine mode: explained inline, not blocked** | `small-q5_1` has no CoreML variant at all (`hasCoreMLSupport == false`). Disabling the "Neural Engine" segment specifically for that model would need a new per-segment-disable capability on the shared `SegmentedControl` component and leaves an orphaned selected-but-disabled state whenever the user switches models. Instead the footnote says so plainly whenever mode is `.on`: the existing `coreMLExplanation` copy gets "— Neural Engine mode has no effect here, this model always runs on the GPU." appended. Matches DESIGN.md's solvable-errors-shown-inline principle over a disabled control. **Follow-up (code review):** the base `coreMLExplanation` copy still read "(CPU only)", flatly contradicting the appended "always runs on the GPU" clause shown right next to it. `coreMLExplanation` now names no chip at all — "…doesn't ship a CoreML encoder — the Neural Engine setting has no effect on this model." A second review pass caught that "always runs on the GPU" was wrong too: with Metal down it is the CPU, and this property has no access to the resolved `AcceleratorCapability` to tell the difference. Naming the chip is the footnote's job, since it does have that input; the base copy states only what holds unconditionally. The mode-specific append in `modelFootnote` was removed as redundant; one sentence, true in all three modes. |
| 2026-07-26 | **Auto mode picks the Neural Engine on every chip, including M5-class hardware with the Metal 4 tensor API — reversing the assumption this whole feature was planned on.** | The plan (and the benchmark it rested on) held that the tensor API beats the ANE on M5, so Auto was built to bypass CoreML there. Re-measuring in the scenario VoiceType actually runs — model resident and warmed, not reloaded per invocation — reversed the result: ANE 192 ms vs Metal+tensor 231 ms on a 5.4 s phrase, 1209 ms vs 1359 ms on 81 s. The original "420 ms vs 855 ms" was measured with `whisper-cli`, which reloads the model on every run and therefore charged each run ~250–435 ms for loading the `.mlmodelc` into the ANE; VoiceType pays that once at launch and hides it behind `performWarmUp()`. The tensor API is still real and still roughly doubles the *Metal* encoder (1213 → 612 ms) — it just does not overtake the ANE. Full write-up and raw numbers: `scripts/bench-output/RESULTS-coreml-vs-metal-app-2026-07-26.md`. User-visible effect: Auto behaves as before this feature, and the segmented control exists so GPU-only remains reachable deliberately rather than by accident. |
| 2026-07-27 | **Accessibility regression fixed: `EvalEditorView`'s local `VoiceTypePrimaryButtonStyle` (white text on `Palette.accent`, ~1.9:1) deleted; `ChecklistPrimaryButtonStyle` from `ButtonStyles.swift` is now the single shared filled-accent button style.** | Same WCAG AA failure already found and fixed once (2026-04-24, black text on accent) had returned via copy-paste into a second file instead of being reused. `ChecklistPrimaryButtonStyle` gained `@Environment(\.isEnabled)` handling (background falls to `Palette.strokeSubtle`, text to `Palette.textMuted` when disabled) because the Eval editor's "Save eval pair" button needed the disabled treatment the checklist-only version never had to support. `EvalEditorView`'s `VoiceTypeSecondaryButtonStyle` (bordered, not filled) has no shared equivalent and stays local — not a duplicate of anything. |
| 2026-07-27 | **`TokensTests.testPrimaryButtonTextContrastMeetsWCAG_AA` added — computes WCAG relative luminance / contrast ratio for black-on-`Palette.accent` in both appearance modes, requires ≥4.5:1, and asserts white-on-accent (the actual regression) would have failed it.** | `TokensTests.swift` checked spacing/radius/motion/window-size constants but never a real `Palette.*` value or anything in `ButtonStyles.swift` — the exact gap that let the white-text regression above ship unnoticed. Test duplicates `Palette.accent`'s dark (`#59C7FF`) / light (`#099DDF`) hex literals rather than resolving the dynamic `Color` at test time, matching the existing `testHexColorParser` pattern in the same file. |
| 2026-07-27 | **`WaveformView.capsuleContent`'s `.errorToast` case now renders `EmptyView()` instead of mirroring `errorInline`'s layout; the Step-7 "render as separate toast NSWindow" TODO removed.** | Dead code: both call sites that touch `CapsuleState.errorToast` (`AppDelegate.showErrorToast()` and the persistent-toast path in `setupServices()`) invoke `stateModel.announcer(...)` directly for the VoiceOver announcement and never assign `stateModel.state = .errorToast(...)` — confirmed by grep, no code path reaches the branch. `ErrorToastWindow` (added the same cycle the TODO was written) is the actual visible surface; the capsule TODO was never revisited after that shipped. The `.errorToast` **case stays in the `CapsuleState` enum** — `CapsuleStateModel.announcementCopy(for:)` still switches on it for VoiceOver copy, and `CapsuleStateTests`/`CapsuleStateModelAnnouncementTests` still exercise it. Only the capsule's visual branch was dead. |
| 2026-07-27 | **`Palette.Artwork.*` (5 tokens) added; `VoiceTypeArtwork.swift`'s 5 inline `Color(red:green:blue:)` literals migrated to them.** | Last `Views/` file predating DESIGN.md (2026-04-12) and the sole source of the 5 `inline_color_rgb` warnings outside `DesignSystem/`. Centralization only — values (dark navy gradient, cyan glow, waveform-bar gradient) are unchanged, this is the app's locked icon artwork, not a repaint. Non-adaptive (`Color`, not `Color.dynamic`) like `Palette.Capsule`, since the icon is a fixed brand mark rather than a themed surface. |
| 2026-07-27 | **`Palette.Toast.{bg,accent,text}` added; `ErrorToastWindow.swift`'s 6 inline colors + file-level `swiftlint:disable inline_color_hex inline_color_rgb inline_nscolor_rgb` removed; its 4 ad-hoc `.system(size:)` fonts replaced with `Typography.body`/`.buttonLabel`/`.caption` (chaining `.weight()` where the exact weight had no matching named token: bold icon, medium "View log" link).** | Values already matched DESIGN.md (`#2A1A1A` toast bg, `#FF7A6B` accent, `#FFDCD5` text) — this closes the outstanding `superfluous_disable_command` warning on that block (some of the 3 disabled rules weren't firing) by removing the disable entirely instead of trimming it. `Palette.Toast.*` is non-adaptive like `Palette.Capsule`/`Palette.Artwork`: the toast is opaque and theme-independent by design, so it must not follow `Palette.error`'s light/dark split even though the values coincide with its dark-mode stop. The former `rgba(255,122,107,0.35)` border literal is now `Palette.Toast.accent.opacity(0.35)` — same RGB, no separate token needed. |
| 2026-07-27 | **`Motion.waveformBarDuration = 0.05` added (new, off the locked micro/short/medium/long scale) for `CapsuleWaveformView`'s bar-height animation; `Motion.micro` reused (not a new token) for the two `SettingsComponents.swift` `0.12`s hover/selection transitions; `SettingsLayout.sidebarWidth = 160` added for `SettingsView`'s sidebar; `MenuBarView`'s hardcoded `9×9` model-status dot now reuses `MenuBar.tallyDotSize` (8pt).** | Four different magic-number situations needed four different resolutions, decided case by case rather than blanket-snapped: (1) the waveform bar's `0.05` is tied to `CapsuleIndicatorView.levelTimer`'s 50ms sampling cadence — a technical sync constant, not a decorative motion choice, so it got its own named token instead of being forced onto `Motion.micro` (100ms), which would make bars visibly lag the live audio level; (2) both `0.12`s are literally "hover feedback, color transitions" — `Motion.micro`'s own DESIGN.md definition — so the 20ms gap was closed by snapping, no new token; (3) the sidebar's `160` was already the correct DESIGN.md value (line 177), just not centralized — plain token extraction; (4) the two dots visible in the same MenuBar dropdown (tally dot 8pt, model-status dot 9pt) had no reason to differ in size and reading as an inconsistency was the actual bug — unified on the existing `tallyDotSize` rather than adding a second dot-size token. |
| 2026-07-27 | **«Normalize terminology» toggle lands in General → INSERTION, directly under «Trim whitespace»; the hallucination filter deliberately gets no toggle at all.** | The lexicon normalizer entered the pipeline (`n1`) with no way for the user to switch it off, while T11 named that toggle a mandatory UX element. Placement follows what the control does: INSERTION is already the "what happens to the text before it is inserted" group, and a user who has just watched «Уголовный кодекс» turn into «Уголовный Codex» looks for the fix next to Trim whitespace, not three tabs away in Advanced. Granularity is deliberately asymmetric: the normalizer rewrites words the user did say — and three dictionary lemmas (`кодекс`, `опус`, `сонет`) are ordinary Russian words, a risk accepted by domain and measured at 0.00% collateral damage on the owner's corpus — so it needs an escape hatch; the hallucination filter removes subtitle boilerplate the user never spoke (19/19 caught, 0 false positives on 1000 records), and there is no reading under which someone wants that inserted, so exposing it would be a control with a wrong setting and no right one. Default is on. Second-order consequence handled with it: the pipeline stamp gains a `-nolex` suffix (`n1-nolex:p…`) when the toggle is off, because the stamp's only job is attributing an entry's text to the pipeline that produced it, and a bare `n1` on an un-normalized entry would be a lie about 29.9% of the corpus. |
| 2026-07-27 | **«Remove filler words» — второй тумблер в General → INSERTION, отдельно от «Normalize terminology»; удаление слов не сводится в один переключатель с заменой терминов.** | Два разных обещания: словарь чинит написание того, что произнесено, а этот слой убирает произнесённое. Цена ошибки тоже разная — неверная замена оставляет странное слово, которое видно и правится, неверное удаление уносит сказанное. Поэтому отключать их хочется порознь, и общий тумблер заставлял бы платить нормализацией терминов за отказ от удаления филлеров. Подпись называет ровно то, что делает слой, включая перечень слов («вот», «ну», «короче») и оговорку «inside sentences»: удаление в начале предложения не производится, и подпись не должна обещать больше правила. **Выключен по умолчанию.** Владелец сначала выбрал включённым (после показа охвата: 43% записей, 1541 удаление на корпусе из 1096 записей), но затем решил отложить выборочный аудит как несрочный — а вместе с ним откладывается и единственная имеющаяся проверка точности на реальной речи. Слой, который удаляет слова, не должен работать непроверенным только потому, что проверка отложена; выключенный по умолчанию тумблер — обратимая сторона. Дефолт переключается на включённый, когда аудит пройдёт гейт (`scripts/filler-audit.py`, 237 случаев) (`scripts/filler-audit.py`, 216 случаев): независимый ревьюер справедливо указал, что holdout корпуса даёт по филлерам четыре наблюдения, и этого мало для default-on, а аудит даёт настоящий доверительный интервал. Штамп конвейера получил вторую пару суффиксов: `n2`, `n2-nofill`, `n2-nolex`, `n2-nofill-nolex` — порядок суффиксов совпадает с порядком стадий, поэтому у каждого состояния ровно одно написание. |
| 2026-07-27 | **Custom Vocabulary field gains a mechanism explainer, a worked example, and a live `N / 223 tokens` counter (`CustomVocabularySection.swift`); the old "Comma- or newline-separated" subtitle is removed as actively harmful.** | First DESIGN.md decision about a per-field character/token counter or input-length limit — no prior precedent existed for this pattern. Owner's ask, verbatim: the field was blank space with no explanation of what to write, what format, or what it buys you. A 41-take measurement (4 arms, deterministic control arm reproduced 41/41 byte-identical) found the removed subtitle was steering users toward the worst format: a comma list ("Codex, Fable, endpoint") collapses transcript-wide punctuation (periods 168→21, commas 364→97 — whisper copies the seed's punctuation style), while the same words as full sentences hold punctuation (periods 239) and score far better on proper nouns (18/43 vs 2/43 with no prompt at all) — though not for free: 6 regressions on already-correct words and some filler-word dropout even in the good arm. The view now states plainly that this is a decoder hint, not a find-and-replace dictionary (that's a separate, unimplemented feature per `README.md:112`), shows the measured-good full-sentence format with a real example, and surfaces the 223-token `initial_prompt` budget via `PromptBudget.evaluate(seed:vocabulary:)` — approximate counts get a leading tilde (`isExact == false`), and an inline warning explains that whisper silently drops the *beginning* on overflow (so important words belong at the end) rather than the app silently truncating the user's text itself. Extracted to its own file, same pattern as `HistorySection.swift`/`DiagnosticsSection.swift`, to keep `SettingsView.swift` off the `file_length` warning path. |
| 2026-07-27 | **Группа MICROPHONE в General: выбор входного устройства. Механизм захвата заменён с `AVAudioRecorder` на `AVCaptureSession` + `AVCaptureAudioDataOutput`.** | `DESIGN.md:185` предусматривал «device select + inline permission hint row» с 2026-04-24, но в коде группы не было: у `AVAudioRecorder` на macOS **нет** выбора входного устройства (`AVAudioSession` — iOS-only, а системное устройство по умолчанию — глобальное состояние, которое приложению менять нельзя). То есть фича была невозможна без замены механизма записи, и вопрос стоял так: чем заменить. `AVAudioEngine` + `setDeviceID`, на котором захват жил до `f6879ca`, **отклонён замером**: `installTap(bufferSize:)` на macOS игнорируется, HAL отдаёт куски по 4800 кадров (100 мс), а волна по DESIGN обновляется каждые 50 мс — половина тиков была бы устаревшей; плюс нативные 48 кГц и обязательная конверсия. Выбранный путь отдаёт куски по 165 кадров (≈10.3 мс, 94 вызова делегата в секунду) и **сразу 16 кГц моно int16**, поэтому конверсия уходит из пути захвата целиком; `stopRunning()` синхронный, так что публичный контракт остановки не изменился; права — та же `AVCaptureDevice`-модель, что уже в `PermissionManager`. Список устройств строится через **CoreAudio**, а не `AVCaptureDevice.DiscoverySession`: последняя показывает агрегатные устройства (`CADefaultDeviceAggregate-…`), которых пользователь не выбирал; UID у обоих API совпадает, поэтому захват берёт устройство тем же uid. Хранится UID, а не `AudioDeviceID` — последний меняется при переподключении. Поведение при исчезнувшем устройстве: пишем с системного по умолчанию и говорим об этом **инлайн-рядом** (модалки запрещены), а выбор в настройках **не затираем** — гарнитуру подключат обратно. Смена устройства применяется к следующей записи, а не к идущей. Прерванная запись не выбрасывается: если что-то записалось, это транскрибируется — человек это произнёс. По умолчанию `nil` = «System Default», то есть поведение до появления пикера. Три раунда plan review Codex: первый снял неверное историческое обоснование, второй — ложную дихотомию с `AVCaptureSession`, третий потребовал 30 циклов на одном экземпляре сервиса и защиту по идентичности output (буфер прошлой записи иначе дописался бы в чужой файл). |
| 2026-09-18 | **`AudioHALGateway` — единственная точка входа к CoreAudio; инвариант «main никогда не вызывает HAL» закреплён lint-правилом `hal_outside_gateway`; системные сбои аудио показываются toast'ом, а не inline-капсулой; в Settings добавлена строка «macOS audio isn't responding».** | `coreaudiod` (macOS 26.6.2) ушёл в deadlock 18.09.2026 — любой первый HAL-вызов из любого потока, включая main, вис навсегда: Settings замораживал всё приложение, а хоткей показывал вводящее в заблуждение «Mic not responding». Решение — своя последовательная очередь с сроком 2 с, состоянием здоровья и fail-fast (`AudioHALGateway`), отдельная от очереди захвата, чтобы одно неисправное устройство не читалось как «система мертва». Системная ошибка — про всю аудиосистему, а не про конкретный микрофон, поэтому capsule (по природе — про запись) её не показывает: toast с командой восстановления. Приложение только информирует, не лечит систему (без внепроцессной пробы, без XPC, без кнопки «перезапустить coreaudiod» — рвёт звук всем и требует пароль). Полный разбор и контракт — `docs/plans/coreaudiod-hang-resilience.md`. |
